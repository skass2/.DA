...
export default function LoginPage() {
  ...
  const [successMsg, setSuccessMsg] = useState(""); // Hiển thị thông báo khi gửi mail thành công
  ...
  const handleStandardAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        // ĐĂNG XUẤT NGAY LẬP TỨC ĐỂ ÉP USER PHẢI VÀO MAIL CLICK LINK MỚI ĐƯỢC LOGIN
        await signOut(auth);
        
        // Hiển thị thông báo thành công và chuyển về màn hình Login
        setIsRegistering(false);
        setPassword("");
        setConfirmPassword("");
        setSuccessMsg(`Đăng ký thành công! Một email xác thực đã được gửi tới ${email}. Vui lòng kiểm tra Hộp thư đến (hoặc thư Rác/Spam) để kích hoạt tài khoản.`);
